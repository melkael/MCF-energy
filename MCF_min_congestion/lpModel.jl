...
function makeModel(sn::MetaDiGraph, paths::Dict, commodities::Vector{Commodity})
    ...
    for i in 1:nv(sn)
        ...
        for K_id in keys(paths)
            for (index, p) in enumerate(paths[K_id])
                ...
                if final_node == i
                    s += vars[K_id][index] * cpus[K_id][index] / (get_prop(sn, i, :cpu_max) - get_prop(sn, i, :cpu_used))
                end
            end
        end
        push!(c_cpu_cong, @constraint(model, congestion >= s))
    end
    ...
    for e in edges(sn)
        ...
        if get_prop(sn, e, :BW_max) < 100000
            for K_id in keys(paths)
                for (index, p) in enumerate(paths[K_id])
                    for edge_of_path in p
                        if e == edge_of_path
                            s += vars[K_id][index] * retrieve_commodity(commodities, K_id).BW_demanded / (get_prop(sn, e, :BW_max) - get_prop(sn, e, :BW_used))
                        end
                    end
                end
            end
        end
        c_BW_cong[e] = @constraint(model, congestion >= s)
    end
    ...
    println("done")
    return model, c_edge, c_commodity, c_node, vars, cpus, artificial_variables, map_vars_paths, constraint_forbidden, edge_contains_paths, congestion, c_cpu_cong, c_BW_cong
end