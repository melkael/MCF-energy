...
function copyProblem(problem::Problem)
    ...
    c_cpu_cong_new = []
    for c in problem.c_cpu_cong
        push!(c_cpu_cong_new, rf[c])
    end

    c_BW_cong_new = Dict()
    for e in keys(problem.c_BW_cong)
        c_BW_cong_new[e] = rf[problem.c_BW_cong[e]]
    end
    ...
    congestion_new = rf[problem.congestion]
    set_start_value(congestion_new, value(problem.congestion))
    ...
    Problem(deepcopy(problem.sn), deepcopy(problem.commodities), deepcopy(problem.paths), model_new, c_edge_new, edge_contains_paths_new ,c_commodity_new, c_node_new, vars_new, cpus_new, artificial_variables_new, map_vars_paths_new, forbidden_arcs_constraints_new, deepcopy(problem.forbidden_arcs), deepcopy(problem.knapsackCoverCutsEdges), deepcopy(problem.knapsackCoverCutsNodes), congestion_new, c_cpu_cong_new, c_BW_cong_new)
