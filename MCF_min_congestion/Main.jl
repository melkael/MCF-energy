...
function main(ARGS)
    ...
    model, c_edge, c_commodity, c_node, vars, cpus, artificial_variables, map_vars_paths, constraint_forbidden, edge_contains_paths, congestion, c_cpu_cong, c_BW_cong = makeModel(sn, paths, commodities)

    #println(paths)
    #stop()
    pb = Problem(sn, commodities, paths, model, c_edge, edge_contains_paths, c_commodity, c_node, vars, cpus, artificial_variables, map_vars_paths, constraint_forbidden, Dict(k.id=>Set() for k in commodities), Dict(i => [] for i in 1:nv(sn)), Dict(i => [] for i in 1:nv(sn)), congestion, c_cpu_cong, c_BW_cong)
