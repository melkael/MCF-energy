...
function main(ARGS)
    ...
    root = NodeBnB(0, floatmax(Float64)-1, 1, nothing, pb, nothing, false, (nothing, nothing))
    ...
    df = DataFrame(CSV.File(ARGS[3]))
    incumbent = df[parse(Int64, ARGS[2]), :][:energy]

    tree = BnBTree(sn, commodities, incumbent, nothing, Inf, q, d, 1, [], -Inf, time(), 3600, false, 0.01, 0, false)
...
df = main(split(ARGS[1] * " " * string(1) * " " * ARGS[4]))
for i in 11:parse(Int64, ARGS[2])
    df = main(split(ARGS[1] * " " * string(i) * " "* ARGS[4]))
    if df != nothing
        append!(dfMain, df)
        @show dfMain
