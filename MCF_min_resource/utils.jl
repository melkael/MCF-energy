...
function copyProblem(problem::Problem)
    ...
    Problem(deepcopy(problem.sn), deepcopy(problem.commodities), deepcopy(problem.paths), model_new, c_edge_new, edge_contains_paths_new ,c_commodity_new, c_node_new, vars_new, cpus_new, artificial_variables_new, map_vars_paths_new, forbidden_arcs_constraints_new, deepcopy(problem.forbidden_arcs), deepcopy(problem.knapsackCoverCutsEdges), deepcopy(problem.knapsackCoverCutsNodes))
...
function length_non_dummy(sn, p)
    nonDummies = 0.0
    for e in p
