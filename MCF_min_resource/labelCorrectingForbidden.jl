...
function addDummyNodes(G, labels)
...
function solvePricing(snWithDual, source, metaDest, forbiddenPaths, dests)
    G = deepcopy(snWithDual)
    labelsObjective = [:sumWandH, :delay]
    G = addDummyNodes(G, labelsObjective)

    labels = labelCorrectingSP(G, source, labelsObjective, [], [], forbiddenPaths) #metaDest+nv(snWithDual))

    #println(retrievePaths(labels, G, metaDest+nv(snWithDual), labelsObjective, [], forbiddenPaths))
    #println(retrievePaths(labels, G, 4, labelsObjective, [], forbiddenPaths))
    #println(metaDest)
    paths = []
    for d in dests
        paths = vcat(paths, retrievePaths(labels, G, d, labelsObjective, [], forbiddenPaths))
    end
    paths_good = []
    for p in paths
        push!(paths_good, vcat(p, metaDest, metaDest+nv(snWithDual)))
    end

    m = map(x->x[1:end-1], paths_good)
